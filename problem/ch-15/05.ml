...
let sep_shortest_eki eki_list =
    let rec get_shortest_eki eki_list = match eki_list with
        [] -> {name = ""; saitan_kyori_km = infinity; path = []}
        | first :: rest -> 
            let shortest_eki = get_shortest_eki rest
            in if first.saitan_kyori_km <= shortest_eki.saitan_kyori_km then first
                                                                        else shortest_eki
    in List.fold_right
        (fun first rest_result ->
            if get_shortest_eki eki_list <> first then first :: rest_result
                                                  else rest_result)
        eki_list
        []
